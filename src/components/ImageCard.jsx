import React from "react";

function ImageCard({ source }) {
  const addToCollection = () => {
    let collection = localStorage.getItem("collection");
    collection = JSON.parse(collection);

    if (collection) {
      if (!collection.includes(source.src.medium)) {
        collection.push(source.src.medium);
      }
      collection = JSON.stringify(collection);
    } else {
      collection = JSON.stringify([source.src.medium]);
    }

    localStorage.setItem("collection", collection);
  };

  return (
    <div className="group relative block rounded-xl overflow-hidden">
      <div className="aspect-w-12 relative aspect-h-7 sm:aspect-none rounded-xl overflow-hidden">
        <button
          onClick={addToCollection}
          className="absolute top-3 right-3 bg-white rounded-full h-10 w-10 z-30 flex items-center justify-center"
          title="add to collaction"
        >
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M12 11C13.1046 11 14 10.1046 14 9C14 7.89543 13.1046 7 12 7C10.8954 7 10 7.89543 10 9C10 10.1046 10.8954 11 12 11Z"
              fill="currentColor"
            />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M18 9C18 11.973 15.8377 14.441 13 14.917V20C13 20.5523 12.5523 21 12 21C11.4477 21 11 20.5523 11 20V14.917C8.16229 14.441 6 11.973 6 9C6 5.68629 8.68629 3 12 3C15.3137 3 18 5.68629 18 9ZM12 13C14.2091 13 16 11.2091 16 9C16 6.79086 14.2091 5 12 5C9.79086 5 8 6.79086 8 9C8 11.2091 9.79086 13 12 13Z"
              fill="currentColor"
            />
          </svg>
        </button>
        <img
          className="group-hover:scale-105 transition-transform duration-500 ease-in-out rounded-xl w-full object-cover"
          src={source.src.medium}
          alt="Image Description"
        />
      </div>
    </div>
  );
}

export default ImageCard;
